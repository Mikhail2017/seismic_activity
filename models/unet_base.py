...
class UNet(model_base.BaseSegmModel):
    ...
    def _build_model(
        ...
        if encoder_type == "vanilla":
            ...
            # Optional ImageNet (or other) backbone init via SMP.
            # Hyperparam names: encoder_weights | pretrained_weights | pretrained.
            # Values: None / "" / "none" → scratch; "imagenet" / "ssl" / … per encoder.
            encoder_weights = hyper_params.get(
                "encoder_weights",
                hyper_params.get(
                    "pretrained_weights",
                    hyper_params.get("pretrained", None),
                ),
            )
            if isinstance(encoder_weights, bool):
                encoder_weights = "imagenet" if encoder_weights else None
            if isinstance(encoder_weights, str):
                encoder_weights = encoder_weights.strip()
                if encoder_weights.lower() in {"", "none", "null", "false", "0"}:
                    encoder_weights = None
            if encoder_weights is not None:
                available = list(
                    smp.encoders.encoders[encoder_type].get("pretrained_settings", {}).keys()
                )
                assert encoder_weights in available, (
                    f"encoder_weights={encoder_weights!r} not available for "
                    f"{encoder_type!r}; choose one of {available} or None"
                )
                logger.info(
                    "Loading SMP encoder %s with pretrained weights=%s (in_channels=%s)",
                    encoder_type,
                    encoder_weights,
                ...
                logger.info(
                    "Building SMP encoder %s from scratch (in_channels=%s)",
                    encoder_type,
                    encoder_input_channels,
                )
            hp_utils.log_hp(
